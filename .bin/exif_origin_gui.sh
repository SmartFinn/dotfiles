...
IFS=, read -r country country_code province_state city location < <(
...
[ -n "$location" ] && exiv2_args+=(
	-M"set Iptc.Application2.SubLocation $location"
	-M"set Xmp.iptc.Location $location"
)

if [ "${#exiv2_args[@]}" -gt 0 ]; then
	msg=$(exiv2 -n "UTF-8" "${exiv2_args[@]}" "$@" 2>&1)
